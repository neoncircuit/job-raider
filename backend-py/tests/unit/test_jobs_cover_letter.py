...
def client(mock_writer, mock_selector, mock_validator):
    ...
    with patch("src.llm.router.create_router", return_value=MagicMock()), patch(
        "src.generation.selector.ResumeSelector", return_value=mock_selector
    ), patch(
        "src.generation.cover_letter_writer.CoverLetterWriter", return_value=mock_writer
    ), patch(
        "src.generation.cover_letter_validator.CoverLetterValidator",
