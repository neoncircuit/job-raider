...
class TestGenerateManualCoverLetter:
    ...
    def test_manual_cover_letter_with_review_records_metadata(self, client):
        """Should include review metadata when ?review=true is passed."""
        resp = client.post(
            "/api/cover-letter/manual?review=true",
            json={
                "title": "Senior Engineer",
                "company": "TechCorp",
                "description": "Build scalable systems with Python and FastAPI. " * 5,
    ...
    def test_manual_cover_letter_with_review_and_deep(self, client):
        """Should compose review and deep flags independently."""
        resp = client.post(
            "/api/cover-letter/manual?review=true&deep=true",
            json={
                "title": "Senior Engineer",
                "company": "TechCorp",
                "description": "Build scalable systems with Python and FastAPI. " * 5,
