...
class SkillAssessment(BaseModel):
    ...
    is_industry_relevant: bool = Field(
        default=True, description="If skill is relevant to target industry"
    )
...
class ExperienceInsight(BaseModel):
    ...
    strengths: List[str] = Field(
        default_factory=list, description="What's strong about this experience"
    )
    gaps: List[str] = Field(default_factory=list, description="Potential gaps")
    achievements_to_highlight: List[str] = Field(
        default_factory=list, description="Key achievements to emphasize"
    )
...
class ProjectInsight(BaseModel):
    ...
    strengths: List[str] = Field(
        default_factory=list, description="What's strong about this project"
    )
    improvements: List[str] = Field(
        default_factory=list, description="Potential improvements"
    )
...
class ResumeAnalysis(BaseModel):
    ...
    key_strengths: List[str] = Field(
        default_factory=list, description="Top 3-5 key strengths"
    )
    key_improvements: List[str] = Field(
        default_factory=list, description="Top 3-5 key improvements"
    )

    # Detailed Sections
    skills_assessment: List[SkillAssessment] = Field(
        default_factory=list, description="Assessment of each skill"
    )
    experience_insights: List[ExperienceInsight] = Field(
        default_factory=list, description="Insights from work experience"
    )
    project_insights: List[ProjectInsight] = Field(
        default_factory=list, description="Insights from projects"
    )

    # Recommendations
    resume_improvements: List[str] = Field(
        default_factory=list, description="Specific resume improvements"
    )
    skill_gaps: List[str] = Field(
        default_factory=list, description="Missing skills to consider developing"
    )
    ...
    target_alignment_score: Optional[float] = Field(
        default=None, description="Alignment with target jobs (0-100)", ge=0, le=100
    )
    competitive_advantages: List[str] = Field(
        default_factory=list, description="Competitive advantages"
    )
    competitive_gaps: List[str] = Field(
        default_factory=list, description="Areas where competition might be stronger"
    )
