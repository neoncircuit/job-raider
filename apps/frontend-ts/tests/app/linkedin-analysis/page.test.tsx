...
vi.mock("@/lib/api/profile", () => ({
  profileApi: {
    ...
    searchLinkedInPeople: (...args: unknown[]) =>
      mockSearchLinkedInPeople(...args),
  },
}));
...
describe("LinkedInAnalysisPage", () => {
  ...
  it("renders all four input tabs", () => {
    ...
    expect(
      screen.getByRole("tab", { name: /linkedin url/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("tab", { name: /search profiles/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("tab", { name: /paste profile text/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("tab", { name: /fill sections manually/i }),
    ).toBeInTheDocument();
  });
  ...
  it("submits LinkedIn URL for analysis", async () => {
    ...
    const analyzeButton = screen.getByRole("button", {
      name: /analyze linkedin profile/i,
    });
  ...
  it("searches for profiles and selects a result", async () => {
    ...
    const keywordsInput = screen.getByPlaceholderText(
      /e\.g\.\, react, typescript/i,
    );
    await user.type(keywordsInput, "engineer");

    const searchButton = screen.getByRole("button", {
      name: /search linkedin profiles/i,
    });
